...
class ValidatorSpec extends ObjectBehavior {
  ...
  // Greater than
  function it_ensures_that_a_numeric_value_is_greater_than_a_given_value() {
    $this->beConstructedWith([ 'price' => [ 'greater_than' => 1789 ]]);
    $this->data([ 'price' => 1920 ]);
    $this->passes()->shouldBe( true );
    $this->errorMessages( 'price' )->shouldHaveCount( 0 );
  }

  function it_fails_to_validate_a_numeric_value_that_is_less_than_the_given_value() {
    $this->beConstructedWith([ 'price' => [ 'greater_than' => 8 ]]);
    $this->data([ 'price' => 7.99 ]);
    $this->passes()->shouldBe( false );
    $this->errorMessages( 'price' )->shouldContain( 'should be greater than 8' );
  }

  function it_fails_to_validate_a_numeric_value_that_is_not_greater_than_but_exactly_the_given_value() {
    $this->beConstructedWith([ 'price' => [ 'greater_than' => 9 ]]);
    $this->data([ 'price' => 9 ]);
    $this->passes()->shouldBe( false );
    $this->errorMessages( 'price' )->shouldContain( 'should be greater than 9' );
  }


  // Greater than or equal to
  function it_ensures_that_a_numeric_value_is_greater_than_or_equal_to_a_given_value() {
    $this->beConstructedWith([ 'price' => [ 'greater_than_or_equal_to' => 101.101 ]]);
    $this->data([ 'price' => 101.101 ]);
    $this->passes()->shouldBe( true );
    $this->errorMessages( 'price' )->shouldHaveCount( 0 );
  }

  function it_fails_to_validate_a_numeric_value_that_is_less_than_and_not_equal_to_the_given_value() {
    $this->beConstructedWith([ 'price' => [ 'greater_than_or_equal_to' => 8 ]]);
    $this->data([ 'price' => 6 ]);
    $this->passes()->shouldBe( false );
    $this->errorMessages( 'price' )->shouldContain( 'should be greater than or equal to 8' );
  }


  // Less than
  function it_ensures_that_a_numeric_value_is_less_than_a_given_value() {
    $this->beConstructedWith([ 'price' => [ 'less_than' => 100 ]]);
    $this->data([ 'price' => 99 ]);
    $this->passes()->shouldBe( true );
    $this->errorMessages( 'price' )->shouldHaveCount( 0 );
  }

  function it_fails_to_validate_a_numeric_value_that_is_greater_than_the_given_value() {
    $this->beConstructedWith([ 'price' => [ 'less_than' => 1 ]]);
    $this->data([ 'price' => 1e5 ]);
    $this->passes()->shouldBe( false );
    $this->errorMessages( 'price' )->shouldContain( 'should be less than 1' );
  }

  function it_fails_to_validate_a_numeric_value_that_is_not_less_but_exactly_the_given_value() {
    $this->beConstructedWith([ 'price' => [ 'less_than' => -10 ]]);
    $this->data([ 'price' => -10 ]);
    $this->passes()->shouldBe( false );
    $this->errorMessages( 'price' )->shouldContain( 'should be less than -10' );
  }


  // Less than or equal to
  function it_ensures_that_a_numeric_value_is_less_than_or_equal_to_a_given_value() {
    $this->beConstructedWith([ 'price' => [ 'less_than_or_equal_to' => 101.101 ]]);
    $this->data([ 'price' => 101.101 ]);
    $this->passes()->shouldBe( true );
    $this->errorMessages( 'price' )->shouldHaveCount( 0 );
  }

  function it_fails_to_validate_a_numeric_value_that_is_greater_than_and_not_equal_to_the_given_value() {
    $this->beConstructedWith([ 'price' => [ 'less_than_or_equal_to' => 8 ]]);
    $this->data([ 'price' => 10 ]);
    $this->passes()->shouldBe( false );
    $this->errorMessages( 'price' )->shouldContain( 'should be less than or equal to 8' );
  }
