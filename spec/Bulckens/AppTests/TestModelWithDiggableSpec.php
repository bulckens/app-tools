...
class TestModelWithDiggableSpec extends ObjectBehavior {
  ...
  function it_gets_a_root_key() {
    $this->get( 'apparatus' )->shouldBe( 'time travel machine' );
  }

  function it_gets_a_nested_key() {
    $this->get( 'first.second.apparatus' )->shouldBe( 'felange' );
  }

  function it_defaults_to_a_given_value_if_the_key_could_not_be_found() {
    $this->get( 'first.second.third.apparatus', 'gyrocar' )->shouldBe( 'gyrocar' );
  }

  function it_returns_the_key_if_explicitly_required() {
    $this->get( 'first.second.third.apparatus', null, true )->shouldBe( 'first.second.third.apparatus' );
  }
